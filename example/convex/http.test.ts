...
describe("room lifecycle", () => {
  test("room_started syncs numParticipants and maxParticipants", async () => {
    const t = initConvexTest();
    await postWebhook(
      t,
      event({
        id: "evt_room_started",
        room: { name: "town-hall", sid: "RM_2", numParticipants: 3, maxParticipants: 100 },
      }),
    );
...
describe("egress lifecycle", () => {
  test("egress_started records the egress row", async () => {
    const t = initConvexTest();
    await postWebhook(
      t,
      event({
        id: "evt_egress_started",
